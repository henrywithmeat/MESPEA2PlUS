...
public class TournamentSelection extends SelectionMethod implements SteadyStateBSourceForm {
    ...
    public int produce(final int subpopulation,
        ...
        Individual[] oldinds = state.population.subpops[subpopulation].individuals;
        int randoma = WSCInitializer.random.nextInt(6);
        double[] newinds = new double[oldinds.length];
        HashMap<Double, Integer> map = new HashMap<>();
        for (int i = 0; i < oldinds.length; i++) {
            SPEA2MultiObjectiveFitness f = (SPEA2MultiObjectiveFitness) oldinds[i].fitness;
            newinds[i] = f.getObjective(randoma);
            map.put(newinds[i], i);
        }
        Arrays.sort(newinds);
        int randomb = WSCInitializer.random.nextInt(10);
        return map.get(newinds[randomb]);
    }
