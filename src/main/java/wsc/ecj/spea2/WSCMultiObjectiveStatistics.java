...
public class WSCMultiObjectiveStatistics extends MultiObjectiveStatistics {
	...
	public void postEvaluationStatistics(final EvolutionState state) {
		...
		for (int x = 0; x < state.population.subpops.length; x++) {
			for (int y = 0; y < state.population.subpops[x].individuals.length; y++) {
				wsc.ecj.nsga2.SequenceVectorIndividual ind = (wsc.ecj.nsga2.SequenceVectorIndividual) state.population.subpops[x].individuals[y];
	...
	public void finalStatistics(final EvolutionState state, final int result) {
		if (!silentFront) {
			...
			double[] ans = {10000,10000,10000,10000,10000,10000};
			...
			for (int s = 0; s < state.population.subpops.length; s++) {
				...
				for (int i = 0; i < sortedFront.length; i++) {
					wsc.ecj.nsga2.SequenceVectorIndividual ind = (wsc.ecj.nsga2.SequenceVectorIndividual) sortedFront[i];
					SPEA2MultiObjectiveFitness f = (SPEA2MultiObjectiveFitness) ind.fitness;
					for(int j = 0;j < 6;j++){
						if(ans[j] > f.getObjective(j))
							ans[j] = f.getObjective(j);
					}
