...
public class NSGA2Evaluator extends SimpleEvaluator {
	...
	public void evaluatePopulation(final EvolutionState state) {
		...
		assignIndi2Subproblem(state);
		// System.out.println("Finish Non-dominated Sorting");

		// assignment of subproblem representative
	}

	public void assignIndi2Subproblem(EvolutionState state) {
		...
		// Sort pop
		int n = pop.individuals.length;
		for (int i = 0; i <= n - 2; i++) {
			for (int j = i + 1; j <= n - 1; j++) {
				MultiObjectiveFitness fit_i = (MultiObjectiveFitness) pop.individuals[i].fitness;
				double f1xi = fit_i.getObjective(0);
				double f2xi = fit_i.getObjective(1);

				MultiObjectiveFitness fit_j = (MultiObjectiveFitness) pop.individuals[j].fitness;
				double f1xj = fit_j.getObjective(0);
				double f2xj = fit_j.getObjective(1);

				if (f2xj < f2xi || ((f2xj == f2xi) && (f1xj > f1xi))) {
					// swap individuals[i] and individuals[j]
					swap(pop.individuals, i, j);
				}
